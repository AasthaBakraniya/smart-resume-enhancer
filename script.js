...
//  4. Show it as a nice resume card
//
//  NEW: It also remembers the conversation so you
//  can "refine" the result with follow-up instructions
//  without retyping your original experience text.
// ─────────────────────────────────────────────

// This array holds the ENTIRE conversation history.
// Every message you send and every reply the AI gives
// gets added here. We send this whole array every time
// we call the API — that's how the AI "remembers."
...
`;
    ...
    // ── Step 3b: Start a FRESH conversation ────────
    // This is a brand new "Enhance" click (not a refine),
    // so we reset history and start counting from scratch.
    conversationHistory = [
        {
            role: 'system',
            content: 'You are a professional resume writer. Always respond with raw JSON only. No markdown. No explanation.'
    ...
    // Run the actual API call (shared with refine())
    await callAI(apiKey, btn, '✨ Enhance with AI');
}


// ─────────────────────────────────────────────
//  refine()
//  Called when user adds a follow-up instruction
//  like "focus more on Laravel and Vue.js"
//  WITHOUT retyping the original experience text
// ─────────────────────────────────────────────
async function refine() {
    ...
    if (!refineText) {
        errorMsg.textContent = '⚠️ Please type what you want to change.';
        return;
    }

    // Safety check — refine only works if there's already
    // a conversation going (i.e. user clicked Enhance first)
    if (conversationHistory.length === 0) {
        errorMsg.textContent = '⚠️ Please click "Enhance with AI" first.';
        return;
    }

    btn.disabled = true;
    btn.innerHTML = '<span class="spinner"></span> Refining...';
    document.getElementById('result').innerHTML = '<div class="empty">⏳ Updating based on your instruction...</div>';

    // ── THE IMPORTANT PART ─────────────────────────
    // We do NOT rewrite the whole prompt.
    // We just ADD a new user message to the existing
    // conversationHistory array. The AI will see
    // everything that came before — original text,
    // its own previous JSON reply, and now this new
    // instruction — all together.
    conversationHistory.push({
        role: 'user',
        content: `Update your previous JSON response based on this instruction: "${refineText}".
Still return ONLY the same JSON structure as before — no explanation, no markdown.`
    });

    await callAI(apiKey, btn, '🔁 Refine');
}


// ─────────────────────────────────────────────
//  callAI()
//  Shared function used by BOTH enhance() and refine()
//  so we don't repeat the same fetch() code twice.
//  It always sends the FULL conversationHistory array.
// ─────────────────────────────────────────────
async function callAI(apiKey, btn, originalBtnText) {
    const errorMsg = document.getElementById('errorMsg');
    ...
    try {
        const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
            ...
            body: JSON.stringify({
                model: 'deepseek/deepseek-v4-flash',   // DeepSeek V4 Flash
                messages: conversationHistory,         // ← send the WHOLE history, not just one prompt
                temperature: 0.3
            })
        });
        ...
        // ── Save the AI's reply into history too! ──────
        // This is critical — next time we call refine(),
        // the AI needs to know what it said last time.
        conversationHistory.push({
            role: 'assistant',
            content: aiReply
        });
        ...
        // Show the refine box now that we have a result
